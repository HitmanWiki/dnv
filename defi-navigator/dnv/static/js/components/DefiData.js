...
function DefiData() {
    ...
    let {
        ...
        error,
        isLoaded
    } = useApiRequest('https://data-api.defipulse.com/api/v1/egs/api/ethgasAPI.json?api-key=19d66f3f573373abd0d393d5b1627b863a61892555ae3d07263f7e93c6d7');
    let {
        data: ethUSDPrice,
        error: erroreth,
        isLoaded: isLoadedEth
    } = useApiRequest(ethUSDPriceURL);
    ...
    return ( <
        ...
        <
        h2 > Gas Prices {
            ethUSDPrice.USD / 1000000000 * gasPricesData.fast
        }
        $ < /h2> <
        div > fast: {
            gasPricesData.fast
        ...
        div > fastest: {
            gasPricesData.fastest
        } < /div> <
        div > safelow: {
            gasPricesData.safeLow
        } < /div> <
        div > average: {
            gasPricesData.average
        } < /div> <
        div > block time: {
            gasPricesData.block_time
        } < /div> 
        ...
        <
        h2 > {
            data.All.dominance_name
        }
        Dominance <
        /h2> <
        span > {
            data.All.dominance_pct
        } % < /span> <
        /div>                <
        /div> <
        ...
        span > {
            data.All.value.total.USD.value
        } < /span> <
        /div>              <
        div className = "tvl-defi" >
        <
        h2 > ETH in DeFi < /h2> <
        span > {
            data.All.value.total.ETH.value
        } < /span> <
        /div>   <
        div className = "tvl-defi" >
        <
        h2 > BTC in DeFi < /h2> <
        span > {
            data.All.value.total.BTC.value
        } < /span> <
        /div>   <
        /div> <
